...
| [0004](0004-result-store.md) | Result Store — the core retains the fetched prefix as compacted columnar segments (scaled-`i64` `NUMBER`, exact-size text, LOB ids); fetch on demand with one batch of read-ahead and bytes per round trip bounded (on 19c one fetch costs O(packets²) in `oracledb` 26.0.0-beta.3); per-result row/byte caps as settings (desktop 1,000,000 rows / 512 MiB, mobile 100,000 / 64 MiB) with an honest limit state; every transaction end ends the store; spill, eviction and Arrow deferred to P3; C ABI 4 shape for M5.2 | Accepted by the lead 2026-09-25 after one independent review round — owner-review points (a)–(e) open: fetch-size sign-off re-asked as a bytes budget, the 1,000,000-row cap vs `SPEC.md` §19, no browsing past the caps, mobile caps, an upstream issue in draft | 2026-09-25 |
