...
- The result store's layout, paging and bounded-memory policy are proposed in
  [ADR-0004](../decisions/0004-result-store.md): compacted columnar segments owned by the core,
  on-demand fetching, and per-result row and byte caps that are user settings.
...
cost well inside every threshold (`docs/exec-plans/active/phase-1-s15-ffi-spike.md`), but the ADR is
not accepted — the owner has not yet ruled on the report's three open questions (K1's p99 wording,
whether K2 means the warm path, K3's baseline/metric), so the boundary's shape remains a measured
proposal, not an accepted decision. Its committed header is
...
2. **FFI mechanism.** Which Rust/C++ interop approach provides a stable, typed, small boundary, and
   ...
   [ADR-0003](../decisions/0003-qt-rust-integration.md) (**Proposed** — spike S15 measured the
   boundary on 2026-09-20 with cost well inside every threshold and no failure located in the
   boundary itself; still not accepted, pending an owner ruling on K1's p99 wording, whether K2 means
   the warm path, and K3's baseline/metric — see
   `docs/exec-plans/active/phase-1-s15-ffi-spike.md`).
3. **Threading and callbacks across FFI — core side settled; FFI event delivery still open.**
   ...
   [ADR-0003](../decisions/0003-qt-rust-integration.md) (**Proposed** — spike S15 measured the event
   delivery path (waker → `invokeMethod` → drain) on 2026-09-20: teardown under a 10,000-iteration
   flood is clean (ASan/UBSan/LSan on Linux CI), and a blocked session costs another session's stream
   nothing measurable. Still not accepted, pending an owner ruling — see
   `docs/exec-plans/active/phase-1-s15-ffi-spike.md`).
...
6. **Result store representation — PROPOSED resolution: [ADR-0004](../decisions/0004-result-store.md)
   (Proposed; lead acceptance and owner review pending).** The core (`db-core`'s `ResultStore`, one
   per open result) retains the fetched prefix as immutable columnar segments, one per fetched
   batch, compacted on the session worker (`NUMBER` as scaled `i64` where exact, exact-size text,
   LOB ids with the locator parked on the worker). The view states demand and the core fetches on
   demand with one batch of read-ahead. Per-result `results.max_rows` / `results.max_bytes` caps are
   user settings (desktop 1,000,000 rows / 512 MiB, mobile 100,000 / 64 MiB) with a typed, honest
   limit state. No eviction; spill and Arrow are deferred to Phase 3 with the benchmarks that would
   reopen them. The C ABI moves to 4: batches are borrowed from the store, not transferred.
...
10. **Crate layout.** Final workspace layout, crate boundaries, and feature flags (provisional in §11).
    ...
    layout was measured, not just built, by spike S15 on 2026-09-20; still not accepted, pending an
    owner ruling on the S15 report's three open questions — see
    `docs/exec-plans/active/phase-1-s15-ffi-spike.md`).
