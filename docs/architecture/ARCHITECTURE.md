...
`crates/ffi` exists and is exercised against the mock driver (M1.3), but ADR-0003 is still
**Proposed**: spike S15 is the gate, and until it runs the boundary's shape is a proposal that
compiles, not an accepted decision. Its committed header is `crates/ffi/include/reldex.h`,
regenerated by `crates/ffi/gen-header.sh`.
