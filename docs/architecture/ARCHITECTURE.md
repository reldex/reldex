...
`crates/ffi/include/reldex.h`, regenerated by `crates/ffi/gen-header.sh`; it reports ABI version 2
(ADR-0003 amendment A11), and §7's "thread affinity" is a real obligation on the adapter rather
than a convention — the rules an adapter must keep are stated per function in that header and
summarised at its top.
