...
**M6.1 as implemented / as measured (branch `phase-1/m6-1-object-browser`, 2026-09-26, `origin/main` `5434f5c`).** Checkbox left for the lead: the model/QML/session/tests are real and green, but the acceptance line's real-database half is blocked by gaps in `crates/ffi` this task's brief explicitly put out of scope (`crates/**`).
