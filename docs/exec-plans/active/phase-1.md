...
9. **What is not read.** Nothing is read after a fetch, commit, rollback, savepoint, ping or close. Nothing is read on a session that needs validation or is lost, or once an abandon has been requested. Output that a function writes while rows are fetched is kept by the server and arrives with the next statement's read, ahead of that statement's own lines. This is measured on the real database.
