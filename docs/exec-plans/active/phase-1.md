...
| M2.15 ★ | `[x]` done 2026-09-27 (PR #48; reviewed twice; ABI 3.2) | FFI pump switch to the M2.5 event queue: real `Terminal` on a loss mid-statement (not only close/failed-open/panic), `abandon` relayed through the real `SessionRegistry` plus an abandoned hint, `EXECUTING`/`TRANSACTION_STATE` events, and server output delivered ahead of the reply that follows it, not only after | `opus` | M2.5, M2.6; ADR-0003 A29 | `crates/ffi/src/session.rs` pump replaced by a `db-core` `SessionEvent` forward | M2.11 | Replaces the thread-per-session interim pump with a direct forward of `db-core`'s `EventQueue`/`Waker` (`phase-1-m2-5-event-queue.md` §3.1/§6/§7.2); every existing D5 ordering guarantee (exactly one reply per accepted request, `Terminal` exactly once) still holds; the C ABI does not change shape, only what it can now deliver. Real concurrency work — review mandatory — deliberately not attempted alongside M2.11's review-round fix pass (ADR-0003 A29). **As implemented (2026-09-26; review round 2026-09-27; ADR-0003 A32–A39):** ABI 3.1 → 3.2, additive. The hub owns one `EventQueue` + `SessionRegistry` and translates on the draining thread; the pump and its thread are gone. New: `EXECUTING`/`TRANSACTION_STATE`/`FETCHED_SEGMENT` kinds, `TERMINAL` with `abandoned` + an owned cause, `reldex_session_abandon` + `ReldexAbandonOutcome`, and mock `SERVER_OUTPUT`/`LOSE_SESSION`/scripted open-ping failure. Fixed on the way: `reldex_hub_session_count`/`reldex_live_counts().sessions` never came down after a normal close; a session now stops counting when its `TERMINAL` is drained. **Review round (PR #48, 2026-09-27), all landed on the branch:** **M1 — 3.2 is now behaviourally additive for a 3.1 caller:** `EXECUTING` carries `request = 0` and the statement's id in a new trailing `executing_request` (only a reply carries a request id), and a caller whose `struct_size` predates 3.2 is never handed — nor woken for — `EXECUTING`/`TRANSACTION_STATE`/`FETCHED_SEGMENT` (A35); the mock's `SERVER_OUTPUT` capability is opt-in as in 3.1. origin/main's 3.1 `smoke.c` built against the 3.2 library passes 457/458; the one miss asserts the library reports 3.1's `sizeof(ReldexEvent)`, which is version-coupled by construction. **S1:** the adapter refuses a library minor below its header's (`Bridge::StartError::AbiMinorTooOld`, tested), because field presence is decided by the minor, not `struct_size` — 3.1's `completed_operation` already sat in 3.0's tail padding (A38). **S2:** stale "fourth gap"/`pump_main` notes fixed; `tst_objectbrowsermodel` asserts the session count drops. **S3 — drain cost (A37):** the boundary's share rose by about 1.1 µs per event on the initial stream and 2.4 µs under `streamscroll` (review A/B: 0.6 → 1.8 µs and 2.2 → 7.0 µs per drain), so the "must not raise the boundary share" criterion is **not met**; the whole-drain criterion **is** (75.6 → 69.7 µs and 232.2 → 235.6 µs). Next step if it ever matters: a budgeted `drain_into` (~0.2 µs/event). Allocations per event taken: `FETCHED` 1 (pinned by a test), `EXECUTING`/`TRANSACTION_STATE`/`COMPLETED`/`SERVER_OUTPUT_CONFIGURED` 0, a query's `EXECUTED` 9, `SERVER_OUTPUT` 5, `TERMINAL` up to 10. **S4:** a lost session's `TERMINAL` clears `transactionPossiblyActive`, and a close answered after `TERMINAL` no longer turns Failed into Closed (tests for both). **S5:** a lost session's column descriptions are kept per session and freed when the caller next names the retired id in close/close_result/abandon (still `NOT_FOUND`); `reldex_live_counts` gained `column_sets`; ABI 4.0 (M5.2 Stage B) will shorten this to "until `TERMINAL` is drained" (A39). **Hand-off (re-review):** `close_result` on a retired lost session frees **all** of that session's orphaned descriptions, not only that result's — the narrower per-result rule is a 4.0 item (A39). The adapter does not name a retired session on its own, so today a lost session's orphans are freed at worksheet close or hub destroy; M3.3 may call close on a lost `TERMINAL` to free them sooner. A wake with nothing to take is rare, not impossible, for any caller: drain until empty. The `crates/ffi/src` line budget was raised to 14,000 in its own commit. **Also (after PR #45 merged):** `ReldexProfileError` — a refused profile's rule crosses as the `CONFIGURATION` error's `native_code`, mirroring `ReldexCredentialError`; `ConnectionManager` does not map it to finer message keys yet. | M |
...
**M2.9 follow-up (found in M2.15's re-review, 2026-09-27; Sonnet):** `CredentialPattern` does not
catch a URL-style `user:secret@host` in the host field — its `UserPasswordPrefix` pattern only
matches `user/password@`. Such text is accepted and stored in the profile (never echoed: the
profile's `Debug` and the refusal message are value-free), so a secret typed that way lands in
SQLite, against P7. Add the pattern, with a test, and decide whether existing profiles need a sweep.
...
**M2.11 notes (as implemented; draft PR #42 open, gates green locally).** The decision record is
[ADR-0003](../../decisions/0003-qt-rust-integration.md)'s "Amendment: M2.11" section (A26–A31).
...
- statement splitting (`reldex_split_statements`) and metadata (`reldex_metadata_prepare` +
  `ReldexMetadataQuery`);
- settings (numeric id, typed value, provenance), profiles (CRUD, credential-pattern refusal),
  connection-parameter composition through the real Oracle `DriverBinding`, credentials
  (`reldex_secret_*`, never a plain string the UI could keep), and history/worksheets/layout — all
  through `ReldexWorkspace`.
...
(ADR-0003 A27). The C smoke harness (`ui/tests/ffi_smoke/smoke.c`) was extended to exercise every
...
`crates/secrets` test — `lock_squat`'s per-user Credential Manager mutex, "accepted platform
limitation (S4)" per the M2.10 notes above — is flaky under concurrent Reldex processes on the same
Windows account, reproduced independently by another task running at the same time; `crates/secrets`
has zero diff on this branch), `crates/ffi/gen-header.sh --check`, `RUSTDOCFLAGS="-D warnings" cargo
doc --workspace --no-deps`, and `bash ui/tests/ffi_smoke/run.sh` (both the C11 and C++17 smoke
targets).
...
| M3.2 | `[x]` done 2026-09-26 (PR #45; three review rounds, see risks) | Connection manager UI: list, create/edit/delete, environment, test-connect | `sonnet` | M2.9 | QML + `ProfileModel` | M2.11 | All `SPEC.md` §17 fields present; environments Dev/Test/UAT/Staging/Production/Custom. **M2.9 hand-off:** endpoint text that looks like a credential is refused with `ProfileError::CredentialInEndpoint { field, pattern }` — show the field and the pattern, never echo the text; offer the "treat as production" choice for a Custom environment only (ADR-0006 P3/P7) **M2.10 hand-off:** offer "save password" only when `CredentialStore::kind().can_store()`, and otherwise save the profile as prompt-each-time. **Write order:** store `put` first, then the `PasswordStorage` flag. If `put` fails (`InvalidSecret`, `TooLarge`, `Locked`, `Denied`, `Backend`), the flag stays prompt-each-time and the UI says so. **Clearing:** delete the stored entry when a profile switches to prompt-each-time, and when the profile is deleted (`NotFound`/`Unavailable` count as done). If the delete fails (`Denied`, `Backend`, `Locked`), the flag still flips to prompt-each-time, the leftover entry is reported to the user, and it is left for M2.14's sweep. Show `CredentialError`'s value-free `Display` (ADR-0007 Consequences). **As implemented (branch `phase-1/m3-2-connection-manager`):** `ConnectionManager` (adapter class, owns a second FFI subsystem — the workspace — with its own waker/drain, exposed as `bridge.connections`) + `ProfileModel` (`QAbstractListModel`, no password role/property anywhere) + `ConnectionManagerDialog.qml` (reached from `Sidebar.qml`'s "Manage Connections..." button). All six environments, the Custom-only production toggle, the credential-pattern refusal (shown via the FFI's own field+pattern message, never the raw text), the write-order/clearing choreography above (with the `Unavailable`-counts-as-done case implemented in the adapter since the FFI only special-cases `NotFound`), and test-connect (via a new `Bridge` hub-sink routing table so it never touches the one worksheet session) are all present. **Known gap:** `ProfileError` carries no per-variant sub-code across the ABI, so every profile/connect-param validation failure — including the credential-pattern refusal — surfaces under one `error.configuration` message key rather than a finer one; deferred to M2.15; see `ui/README.md` "Connection manager (M3.2)" → "FFI gaps found". **Test-harness bug found and fixed during this task (not an FFI/product bug):** `tst_connectionmanager`'s per-test `EnvGuard` scope-guards were replaced via plain `unique_ptr` assignment in `init()`; `unique_ptr::reset()`'s "assign the new pointer, then delete the old one" order meant the previous test's guard destructor ran *after* the next test's guard had already set `RELDEX_WORKSPACE_IN_MEMORY=1`, undoing it on every other test — silently opening the developer's real on-disk default store instead of an isolated in-memory one (caught because it surfaced as a flaky-looking `rowCount` mismatch only in the Release configuration; root-caused with a temporary diagnostic print showing `forceInMemory` alternating 1/0/1/0 across declared test order). Fixed by explicitly destroying the old guards before constructing new ones in `init()`; the accidental on-disk store this created on the development machine during the task was deleted. **M3.2 fix round (2026-09-26), independent review returned RETURN with four must-fixes, all landed on the same branch:** (1) real-store contamination — `ConnectionManager` no longer opens the workspace from its constructor; `open()` is now explicit/idempotent, called once from `Main.qml`'s `Component.onCompleted`, with a three-layer guard (lazy open; a harness-level `RELDEX_WORKSPACE_IN_MEMORY=1` default in `AdapterTestSupport.h` that runs before any test's `main()`; a CI-detectable before/after store-mtime guard in `ui/build.sh --test`) — verified end to end against the real on-disk store, mtime unchanged throughout; (2) `reldex_workspace_open` now creates the store's directory/file itself, with ADR-0006 P5 permissions, on its own service thread (`Store::open_creating`, additive-only ABI, `crates/ffi/gen-header.sh --check` clean) — `ConnectionManager::ensureStoreDirectoryReady()` no longer exists; (3) the dialog's docs falsely claimed a disabled-with-a-note transport picker that was never actually shipped — corrected, and a session-role picker (Normal/SYSDBA/SYSOPER) was added; TCPS/CA/cert-pin stay M3.5's (see that row); (4) `authFailureWithStoredPassword` used to fire on any authentication failure regardless of where the password came from — `testConnect()` now asks `resolve_password` first and only counts it when the result was `FromStore`. Should-fixes also landed: an offscreen/DPI test for the dialog (`tst_coreinfo.cpp`, 1x + 2x), which caught and fixed a real pre-existing bug (the dialog `Popup` rendered ~200px wide because it inherited its declaring parent's, i.e. the 240px sidebar's, width instead of the window's — fixed with `parent: Overlay.overlay`); the typed password is now cleared from the dialog's own state immediately after test-connect/save reads it; Enter/Return now activates Save. `ui/build.sh --test` and `--release --test` both green (11/11 ctest, plus the new store guard). Lead: please review before flipping the checkbox. | L |
| M3.3 ★ | `[ ]` todo | Connect flow over the async path, with a bounded timeout and a cancellable "Connecting…" state | `opus` | §B3 | `SessionController` | M2.6, M2.11 | Cancelling a pending connect returns immediately and adopts nothing late; failures show kind + ORA code + cause chain **M2.10 hand-off:** get the password with `reldex_secrets::resolve_password` on the workspace service thread, never the UI thread, and handle `FromStore` / `PromptRequired(reason)` / `NotNeeded` by name. On a prompt, say why (e.g. `StoreFailed(Malformed)`: the saved entry is not one Reldex wrote). **Saving:** save a typed password only after the connect succeeds, only if the user asked, and follow M3.2's write order. **Refused stored password:** if the database refuses a `FromStore` password, the next connect prompts and the UI offers to update the saved password. **Never retry a stored password automatically:** each attempt counts towards the account's failed-login limit. **No store:** on every platform except Windows today, every connect prompts (ADR-0007 S3, Consequences). **M2.15 hand-off:** cancel a pending connect with `reldex_session_abandon`. It returns at once with `CONNECTING`, and the late `OPENED` carries a `Cancelled` error, followed by `TERMINAL` with `abandoned`. `SessionController::handleTerminal` already routes `TERMINAL` (Failed on loss, surfacing `transaction_possibly_lost`). A retired id is `NOT_FOUND`. `EXECUTING` carries a live request id, so it must never touch request bookkeeping. Test failed connects/pings with `ReldexMockScenarioConfig`'s scripted failure, not a database. **M3.4 hand-off:** call `bridge.session.setActiveProfileIsProduction(...)` with the connecting profile's `treatAsProduction` once the session actually opens against a profile, and set it back to `false` on close/disconnect — see `ui/README.md` "Production indicator (M3.4)" → "Hand-off". **As implemented (2026-09-27; ADR-0003 A40–A43; `ui/README.md` "Connect flow (M3.3)"):** ABI 3.2 → 3.3, additive: `RELDEX_DRIVER_KIND_ORACLE`; a trailing `ReldexOpenOptions::connect`; `reldex_workspace_prepare_connect`, whose reply kind is `CONNECT_PREPARED` and which adds the password source `SUPPLIED`; and a `ReldexConnectSummary` that now carries the `ConnectionParams` it summarises, password included and wiped on release. The last change resolves A31. The Oracle session opens through the same `SessionRegistry` as the mock, and `mock.rs` is still the one place that picks the driver. Adapter: `SessionController::connectState` (NotConnected → Preparing → [AwaitingPassword] → Connecting → Connected → Disconnecting; ConnectFailed/TimedOut/Cancelled/Lost); `ConnectionManager::prepareConnect`/`saveConnectPassword` and its memory of refused stored passwords. QML: a Connect button per sidebar row, `ConnectPasswordDialog`, the connection state with Cancel and Disconnect in the status bar, and `DisconnectConfirmDialog`. The production indicator is set from the profile's `treatAsProduction` when the session opens and cleared on close or loss (M3.4 hand-off done, tested). **Numbers** (AMD Ryzen 7 5700G, Windows 11, the local 19c container). FFI (`m3_3_connect_live`, n=10), release: prepare p50 224 µs; open → `OPENED` p50 51.7 ms (max 171.6 ms); first reply (`select 1 from dual`) p50 1.72 ms. The same, debug: prepare 0.33–0.41 ms; open 143–165 ms; first reply 1.5–1.9 ms. UI end to end (`connectFlowAgainstTheRealDatabase`, RelWithDebInfo, n=5, four runs): connect p50 46/48/49/47 ms; `select 1` 2/2/3/3 ms; longest UI-thread call 226/264/314/430 µs. Abandon mid-connect returns `CONNECTING` in 18–105 µs. The late connection still reaches the server: a `v$session` watch saw at most one of the test user's sessions. It was closed within the 3 s watch, nothing was left behind, and the watch passed its control check. An unroutable host with a 2 s limit fails at 2.00–2.015 s. ORA-01017 arrives as `AUTHENTICATION` with native code 1017. Auto-commit off is proven with a second session. **K-ruling assumptions:** nothing here measures, reinterprets or depends on K1/K2/K3, which stay the owner's to rule. The timeout is adapter policy, not ABI. The adapter's timer at the resolved limit abandons and reports `TIMEOUT`, and a driver `CONNECTION` error at or after the limit also reads as timed out. That assumes the driver never reports its limit early, which was never seen. **Known limitations:** (1) a statement deadline that fires loses the session: `NETWORK_LOST` at ~3.0 s for a 1.5 s deadline (ADR-0002 T4), and cancel answers `NOT_INTERRUPTIBLE`; (2) after any non-DDL statement, a `SELECT` included, the thin driver cannot rule out a transaction, so Disconnect always asks after a query; (3) with no credential store, every connect prompts (`StoreUnavailable`). `with_no_credential_store_every_connect_prompts` proves it through `prepare_connect`, but only on the CI platforms that have no store (Linux, macOS), not on Windows. `StoreFailed` rests on `reldex_secrets`' `resolve_password` truth table and the reply mapping `prepare_connect` shares with `resolve_password`. Neither prompt is driven through the adapter, whose tests always use the memory store; (4) the `QString` a Qt `TextField` holds cannot be wiped; (5) the memory of a refused stored password lasts for the run only; (6) `ConnectionManager::testConnect()` still opens the mock. **Hand-offs:** **M6.1 real-DB half.** The Oracle driver kind now exists. The browser's own session still opens the mock through `open()` and must move to `prepare_connect` → `ORACLE`. Binds on `reldex_session_execute`, which have no task row yet, still block real metadata rows. **M2.14.** M3.3 writes a credential only through M3.2's put-then-flip (`saveConnectPassword`). If the put lands but the profile update fails, the entry stays for a profile that still prompts. It is not an orphan by the UUID rule, so decide whether the sweep covers it. **M3.5.** `prepare_connect` already carries the TLS mode and CA directory through `connection_params`, so TCPS needs no new crossing. Only plain TCP was exercised here, so add a live TCPS connect through the same path (`RELDEX_TEST_ORACLE_TCPS_DSN`). **M4.9.** The connect flow is already per controller, because `prepareConnect` routes each reply to the controller that asked. With one `SessionController` per tab, the password dialog and status bar (bound to `bridge.session` in `Main.qml`) must follow the active tab. **Follow-up (no row):** move `testConnect()` onto `prepare_connect` + `ORACLE`. | M |
| M3.4 | `[x]` done 2026-09-27 (PR #49; reviewed; re-check at M4.1/M4.9) | Production indicator: persistent, not colour-only (icon + text + tab badge) | `sonnet` | SPEC §17 | QML | M3.2 | Visible in every place a statement can be run; passes a greyscale check. **M2.9 hand-off:** read `Profile::treat_as_production()`, not the `Environment` enum — always on for Production, the user's choice for Custom (ADR-0006 P3). **As implemented:** `SessionController::activeProfileIsProduction` (new `Q_PROPERTY(bool ...)`, default `false`) carries `treat_as_production()` across the adapter boundary; `ui/app/ProductionIndicator.qml` (icon glyph + text label, both `Theme.tokens.warning`, never colour alone) is placed in the status bar (replacing the M3.1 reserved slot), a new worksheet-header strip above the editor/result split, and each worksheet tab (overlaid on `TabButton`, `compact` form). All three read one shared `productionActive` bool `Main.qml` passes down from `bridge.session.activeProfileIsProduction` — no file below `Main.qml` reaches into `bridge` itself. **Deviation from the M2.9 hand-off's original wording** ("read `treatAsProduction` off `ProfileModel`'s role/`get()` map for the active worksheet's profile" — see `ui/README.md`'s old M3.2 "Hand-off" bullet): implemented as a direct bool on `SessionController` instead of a QML-side `ProfileModel` lookup by profile id, because the lookup would have made a QML binding responsible for joining two adapter objects to decide something, one step past "display what the adapter decided." M3.3 does not exist yet, so nothing sets this property in the shipped app today; tests exercise it by calling the setter directly. Tests: `ui/tests/tst_coreinfo.cpp` — `productionIndicatorHiddenByDefault`, `productionIndicatorVisibleInAllThreePlacesWhenActive`, `productionIndicatorFollowsTheFlagNotTheEnvironment` (stands in for Custom on/off), `productionIndicatorPassesGreyscaleCheck` (greyscale luminance-contrast assertion against the status bar's own background, not a pixel-diff), `productionIndicatorRendersAtCurrentScaleFactor` (1x + 2x, `ui/tests/CMakeLists.txt`'s `tst_productionindicator_dpi200`). See `ui/README.md` "Production indicator (M3.4)" for the full design and its own "Hand-off" to M3.3/M4.9/M6.4. | S |
| M3.5 ★ | `[ ]` todo | TCPS UI described exactly as `SPEC.md` §8: user-supplied CA PEM, verification always on; surfaces the descriptor-guard warnings from C-6 | `opus` | SPEC §8; spike S8; PR #5 | QML + wording | M2.3 | No control implies mTLS, wallet files, OS trust store or revocation; `SSL_SERVER_CERT_DN` refusal explains the opt-out rather than failing blankly. **M3.2 fix round note (2026-09-26):** `ConnectionManagerDialog.qml` has no transport/CA/certificate-pin controls at all yet (every profile is created `transport: Plain` with an empty `caDirectory`) — a short caption in that dialog already says TCPS is not yet configurable; this milestone adds the transport picker, the CA PEM input, verification-always-on, and the `SSL_SERVER_CERT_DN` guard/warning surface on top of it. | M |
...
| M3.7 ★ | `[ ]` todo | Logging/diagnostics: `tracing` + rotating file sink, redaction layer, Qt messages forwarded through the FFI | `opus` | AGENTS "do not log secrets" | `crates/ffi` + core | M2.11 | A test asserts no password, PEM, or token reaches the log at any level; SQL text logged only at `debug` behind an explicit opt-in; connection strings redacted. **M2.9 hand-off:** two connect-string echoes remain outside `reldex-workspace` (whose own `Debug` prints only a length): `ConnectionParams`' derived `Debug` (`db-driver-api` `params.rs:228`) and upstream `oracledb`'s `invalid connect string: {connect_string}: {reason}` **M2.10 hand-off:** `CredentialError`, `PromptReason`, `CredentialStoreKind` and the stores' `Debug` are value-free, and `PasswordSource` prints `Secret(<redacted>)`. The log test should include a credential-store round trip with a marker password (ADR-0007). **M2.15 hand-off:** `crates/ffi` itself logs nothing. `TERMINAL` now carries the loss cause as a `ReldexError` whose message is driver text, so it goes through the redaction layer like any other error. The threads to name in diagnostics are `db-core`'s session workers and the short-lived `reldex-ffi-teardown` (destroy with sessions still finishing). | M |
...
| M4.7 | `[ ]` todo | DBMS_OUTPUT pane: per-worksheet enable, size, clear, truncation notice | `sonnet` | M2.7 | QML | M2.7 | Off by default (it costs a round trip); truncation is reported, never silent. The pane must also say that the user's own `DBMS_OUTPUT` calls override it: an `ENABLE(2000)` in the user's code overflows at 2,000 bytes even under an "unlimited" pane, and a `DISABLE` in the user's code purges what was buffered. **M2.15 hand-off:** `SERVER_OUTPUT` now arrives ahead of the reply that follows it, with `request == 0`, so route it by session. One statement's output may span several events. Each event owns its `server_output_lines` (release with `reldex_server_output_lines_release`; today's `Bridge` releases them unshown). The pane must show three things: `server_output_dropped` (per-session cap of 256 undrained unsolicited events, also reported on `TERMINAL`), `server_output_invalid_utf8_lines`, and a non-null `error`, which means a read failed and the output is incomplete. | S |
...
| M4.10 | `[~]` in progress — store side done 2026-09-25 (reviewed; O(1) trim; schema v4); UI half pending M2.11/M4.x | Query history (per profile, SQLite), re-run into the current worksheet | `sonnet` | M2.9 | Store + QML | M2.9 | Bounded size; secrets never captured; text stored verbatim | S |
...
- [ ] M2.11 exposes `record_history`/`history`/`clear_history` and the `HistoryId`/`HistoryOutcome`/`HistoryPage` types over the FFI. (Delivered as `reldex_workspace_record_history`/`reldex_workspace_list_history`/`reldex_workspace_clear_history` and `ReldexHistoryOutcomeKind`/`ReldexHistoryList`/`ReldexHistoryRecordView` — see "M2.11 notes" above; checkbox left for the lead pending review.)
...
**M4.10 fix round (2026-09-25, PR #38 independent review).** The trim's cost was O(min(rows, limit)) per insert, not O(1) — flat at a small limit but growing to tens of milliseconds per insert at a large one as the table grew (measured up to 30 ms/insert at 50,000 rows, limit 100,000/1,000,000). Fixed with schema 4's `history_meta` per-profile counter: a steady-state insert now deletes exactly the one row pushed past the bound, by an indexed point delete, independent of both the limit and the table's size — measured flat (~21–50 µs in-memory, ~530–580 µs on a real file) across every tested limit including the shipped 1,000,000, so that bound is kept, not shrunk. Full design and the before/after measurement table are in the ADR-0006 amendment's "P9 amendment" subsection. Also fixed in the same round: the `SchemaMismatch` error mapping narrowed to the message shapes that actually prove a schema difference (was mapping every bare SQLite prepare failure, risking mislabelling a future Reldex-side SQL bug); three "ADR-0002 E7" citations corrected to `SPEC.md` §20/§24.16 and ADR-0002 D2 (E7 is about idempotent close reporting, a different concern); `Debug` on `HistoryEntry`/`HistoryRecord`/`WorksheetState` now redacts the statement/worksheet text.
...
| ID | Status | Title | Owner | Inputs | Outputs | Deps | Acceptance | Size |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| M5.1 ★ | `[x]` done 2026-09-25 — ADR-0004 Accepted by the lead (reviewed; doc-only fix round landed); owner-review points listed | **ADR-0004 — Result Store representation, paging and bounded-memory policy** | `opus` | ARCHITECTURE §13 item 6; spike S14 | `docs/decisions/0004-result-store.md` | M1 gate | Decides: retain the fetched prefix in core, fetch-on-demand as the view scrolls, configurable row/byte caps with an explicit "fetched N rows (limit reached)" state; spill/eviction and Arrow explicitly deferred to P3 with the benchmark that would reopen it. **As written (2026-09-25; Accepted by the lead after one independent review round, PR #39; owner-review points (a)–(e) open):** [ADR-0004](../../decisions/0004-result-store.md) RS1–RS6 — core-owned compacted columnar segments (scaled-`i64` `NUMBER`, exact-size text, LOB ids); the view states demand and the core fetches with one batch of read-ahead, **bytes per round trip** bounded from the describe's declared widths (on 19c `oracledb` 26.0.0-beta.3 costs O(packets²) per fetch: 16 KB rows take 23 s per 1,000-row fetch); `results.max_rows`/`results.max_bytes`/`results.close_cursor_at_limit` desktop 1,000,000 / 512 MiB, mobile 100,000 / 64 MiB, cursor kept open at the cap until the transaction ends; every transaction end ends the store; no eviction, spill and Arrow deferred to P3 with reopening benchmarks; C ABI 4 shape for M5.2 (no mirror cache). Evidence: `phase-1-m5-1-data/` (with a README) and `crates/db-core/benches/result_store_shapes.rs` | M |
| M5.2 ★ | `[~]` in progress — Stage A (core) done 2026-09-26 (reviewed; live 5/5); Stage B (FFI, ABI 4) after M2.11/M2.15 | Result Store implementation in `db-core` + FFI batch lifetime rules | `opus` | M5.1 | `db-core` + `crates/ffi` | M5.1 | Random access O(1) over the fetched prefix; batch release rules tested; no locator ever crosses a thread (K1 test extended). **Follow-up from ADR-0004's review (no new id):** `db-core` releases results after a successful typed `TransactionControl` statement exactly as after the commit/rollback commands (ADR-0002 amendment X1) — lands with M5.2 or M4.3, whichever is first. **Stage A, the core, is implemented (2026-09-26, branch `phase-1/m5-2-result-store`, draft PR; Stage B, ABI 4 in `crates/ffi`, not started):** `db-core`'s `store` module — `ResultSegment` compacted on the worker (scaled-`i64` `NUMBER`, text copied to size, LOB ids), `ResultStore` (demand fetch with one round trip of read-ahead sized by a 256 KiB budget, exact row cap with lookahead, byte cap, sequence-checked replies, typed `ResultState`), `SessionResults` (every transaction end and the session's end end the stores); X1 implemented; the three result-cap settings registered. O(1) lookup while segments between the first and the last are uniform, binary search otherwise (limitation 10). Tests: unit tests for every transition, mock integration tests on both reply paths plus `SessionResults` on the event path, K1 extended, five live 19c tests. Bench re-run: bytes/row within +0.66% of ADR-0004 Table 1, private bytes +3.2–5.0% over accounted (`phase-1-m5-2-data/`). Accepted limitation 11 stands (no driver mechanism chosen); ADR-0004 "As implemented" lists every deviation. Independent review of PR #44: ACCEPT-WITH-FOLLOW-UPS; the follow-ups landed on the same branch (session loss mid-fetch keeps `Failed` with LOB reason `SessionEnded`; the parked-LOB map gives its storage back; LOB charge 320 B; failed DDL/`COMMIT` that still end the transaction recorded as limitation 13 and owner point (f); Stage B keeps the 100-row wire array, limitation 12 and owner point (g)). **Trap for Stage B (M5.6):** `StatementSettings::apply` (`crates/workspace/src/connect.rs`) already sends `results.fetch_rows` unconditionally as the wire array size and is unused on the product path; Stage B must bound the wire array by `results.round_trip_bytes` and the row's width instead of wiring that method in as it stands | L |
...
| M5.5 | `[ ]` todo | CLOB/BLOB viewers over `read_lob_chunk`, paged, with a size warning | `sonnet` | `db-core` LOB API | QML | M5.2 | 100 MB CLOB opens with bounded memory (S7 method); NCLOB Thai/non-BMP byte-exact (S11 method). **From the M5.2 review:** measure and bound the client-side footprint of parked LOBs (the worker's map, the driver's locator, the 64 KiB staging buffer of a LOB that was read) and replace the Result Store's per-LOB charge — 320 B, from one rough measurement of about 288 B per temporary CLOB — with the measured figure | M |
| M5.6 ★ | `[x]` done | **Fetch-batch benchmark** across row shapes and a real network; pick and record the shipped default | `opus` | spike S14 | `docs/exec-plans/active/phase-1-fetch-benchmark.md` | M5.2 | ≥3 row shapes × ≥4 batch sizes × local and a latency-injected link; method and environment recorded; the default is a *setting* with a bounded range, and the number is justified by the data (S14 showed throughput is not monotonic). **As measured (2026-09-26, branch `phase-1/m5-6-fetch-benchmark`, draft PR; review pending; checkbox for the lead):** [`phase-1-fetch-benchmark.md`](phase-1-fetch-benchmark.md) — ignored live harness in `oracle-thin` (`tests/m5_6_fetch_benchmark.rs`, std-only TNS-counting latency relay), 3 shapes × 8–9 sizes × 5 links (loopback, relay +0/2/10/40 ms) × 3 runs = 375 runs on 19.3 (protocol 318, SDU 8,192), plus CLOB and bandwidth-limited supplements. O(packets²) quantified: `c` = 0.076 / 0.012 / 0.0023 ms per packet² (narrow / mixed / 16 KB rows), about 1 µs per row and 0.5–0.7 µs per KB per re-parse. **Proposed default: 192 KiB per round trip** (new setting `results.round_trip_bytes`, 16 KiB–4 MiB, no "no limit", all levels) **with `results.fetch_rows` = 1,000 as the row bound** (unchanged); mixed shape 88% of best on loopback and 97% at 10 ms; latency ceiling 50 ms p50 per round trip on loopback, 100 ms at 10 ms (default: 10.9 / 21.2 ms). Wide rows pay: 56% at 10 ms, 36% at 40 ms (a profile can raise the budget). The wire array stays 100 rows (ADR-0004 limitation 12, unchanged); its cost at 10 ms is 12.8 s against 2.1 s per 100,000 narrow rows. Not measured: a real WAN, 23ai, mobile. ADR-0004 "As measured: M5.6", ADR-0006 amendment "The round-trip budget (M5.6)"; owner sign-off pending (§C.3 item 10) | M |
...
| ID | Status | Title | Owner | Inputs | Outputs | Deps | Acceptance | Size |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| M6.1 | `[~]` in progress — model/QML/tests done 2026-09-26; review-found refresh/filter race fixed; real-DB half (permission message, large-schema measurement, row cap as a new SettingId) after M3.3 | Object browser: lazy tree over the 9 `SPEC.md` §16 groups, server-side filter, row cap, columns of a selected table | `sonnet` | M2.8 | QML + model | M2.8 | Uses its own metadata session (never the worksheet's); a permission failure reads as a permission message, not a driver error; no freeze with a large schema (measured) | L |
| M6.2 | `[~]` in progress — store side done 2026-09-25 (reviewed); shell half pending M2.11/M3.x | Workspace persistence: open worksheets, text, layout, active profile — **non-transactional state only** | `sonnet` | SPEC §20/§24.16 | Store + shell | M2.9 | Restores after restart; never restores or implies a session or a transaction | M |
...
| M6.9 | `[x]` done 2026-09-25 — measured, no change adopted; fresh baseline 576 ms median; K2 recommendation unchanged; pre-warm delegate deferred to M4.x | Cold first paint ≈ 800–900 ms (D3D11 device creation ≈ 250 ms + first delegate-instantiation polish ≈ 551 ms) vs `SPEC.md` §19 startup target — investigate fix candidates named in the S15 report | `sonnet` | S15 K2 diagnosis (`phase-1-s15-ffi-spike.md`) | Adapter/QML startup change + re-measurement | M1 gate | Cold execute → first painted frame materially under 903.55 ms, ideally toward `SPEC.md` §19's <1 s desirable / <2 s acceptable warm-startup target; warm-path number (15.85 ms) unaffected | M |

**Parallelism.** M6.1/M6.2 (features) run alongside M6.3/M6.4 (non-functional), M6.5/M6.7 (build) and M6.9 (startup). **Review:** M6.6, M6.8.

**M6.1 as implemented / as measured (branch `phase-1/m6-1-object-browser`, 2026-09-26, `origin/main` `5434f5c`).** Marked in progress by the lead 2026-09-26 after the review fix round (PR #47); the model/QML/session/tests are real and green, but the acceptance line's real-database half is blocked by gaps in `crates/ffi` this task's brief explicitly put out of scope (`crates/**`).

- **Delivered:** `ui/adapter/ObjectBrowserModel.{h,cpp}` — a lazy `QAbstractItemModel` tree, connection → schema → the 9 groups → objects → (table/view) a `Columns` node — with its **own** `SessionController`/session, opened lazily on first expand and closed (via `RELDEX_CLOSE_DISPOSITION_ROLLBACK` — see below) on `bridge` change or destruction, never the worksheet's (`bridge.session()`). `ui/app/ObjectBrowserPanel.qml`, wired into `Sidebar.qml` (replacing its M3.1 placeholder tree) via one new `bridge` property; `Main.qml` gained the one `Bridge` instance both this and M3.2 are expected to share. `ui/tests/tst_objectbrowsermodel.cpp` (12 tests, offscreen, no QML engine) and the existing `tst_coreinfo.cpp` QML suite (which now also exercises the new surface as a side effect of loading `Main.qml`) are both green.
- **FFI gap found and worked around honestly, not silently guessed past** (full detail with file/line references: `ui/README.md` "Object browser (M6.1)"): `reldex_session_execute()` cannot carry the bind values every metadata statement requires (`crates/ffi/src/session.rs`: "Binds are not exported yet (M2.11)"); the only mock scenario `reldex_hub_open_session()` can open (`S14`) has no metadata fixtures wired into it at the FFI layer (`crates/ffi/src/mock.rs`'s `build_driver()` never calls `MetadataFixture`); and `ReldexDriverKind` has no non-mock value at all yet, so there is no Oracle session to open from this FFI regardless of the first two gaps. The model's prepare → execute → classify pipeline is genuinely real and wired end to end; it cannot currently return real rows against *any* driver this build can open, and fails safely (a typed, generic message, never a crash, never invented data) when it tries — proven by a real-FFI test, not asserted from reading the source. `reldex_metadata_prepare`/`_reclassify_error` (no session involved) are proven for real: a synthetic ORA-00942 is turned into `Permission` through the actual Oracle classifier.
- **"No freeze" measurement: mock-fed, in-process only, not against the container.** Even with `reldex-oracle19c` healthy (checked twice: `docker ps` showed `Up 29 hours (healthy)` both when this was first written and again when the numbers below were recorded), this build cannot run a single metadata query against it — no `ReldexDriverKind` names Oracle yet, so there is no session to open, independent of the bind-support gap. Measured instead: `applyRows()` (the same tree-building code a real fetch would call) fed 5,000 synthetic, distinct table rows directly, then narrowed to 100 ("filter"), `QElapsedTimer` around each call, on the UI thread in real use (`tst_objectbrowsermodel.cpp`'s `applyingFiveThousandObjectsDoesNotBlockTheUiThreadForLong`, opt-in via `RELDEX_UI_SANITY_OBJECTS=1`, `qInfo()`-reported). Recorded 2026-09-26, AMD Ryzen 7 5700G / ~63 GB RAM / Windows 11, RelWithDebInfo build, idle machine: 3 process invocations × 3 runs each (9 samples) — expand 5,000 rows: 1–3 ms; filter to 100 rows: 0–1 ms every time (full table in `ui/README.md` "Object browser (M6.1)" → "The 'no freeze' measurement"). This is a real measurement of the actual freeze mechanism (a large synchronous tree rebuild on the UI thread), not a placeholder — but it is not the container-based, full-`TreeView`-paint measurement the acceptance line asks for, because that measurement cannot be taken at all until the gaps above close.
- **A fourth FFI gap found while testing this task's own session close, unrelated to the three above (fixed by M2.15, ABI 3.2: a session stops counting when its `TERMINAL` is drained, and `tst_objectbrowsermodel` now asserts the count drops):** `reldex_hub_session_count()` (and process-wide `reldex_live_counts().sessions`) does not decrement when a session closes normally on a hub that is not itself being destroyed — confirmed against the raw FFI with a throwaway Rust test (not committed, `crates/**` out of scope): even the *only* session on a hub, closed cleanly with both its `SESSION_CLOSED` and unsolicited `TERMINAL` events drained, leaves the count unchanged, contradicting `reldex_hub_session_count()`'s own doc comment in `reldex.h`. Root cause (read-only, not touched): `reldex_hub_open_session` inserts into `hub.sessions` (`crates/ffi/src/session.rs` ~line 625) but no normal-close path ever removes that entry — only `reldex_hub_destroy`'s full drain and the failed-thread-spawn error path do. This is what first looked like a bug in this task's own close code (a 60-second test hang); it is not — the adapter's close protocol is correct and prompt, and `tst_objectbrowsermodel.cpp`'s close test was rewritten to assert on `SessionController`'s own state/signal instead of this count. Full write-up: `ui/README.md` "Object browser (M6.1)" → "A fourth gap, found while testing this task's own session close". Flagging this because it is not scoped to M6.1: anything else that waits on this count or on `reldex_live_counts().sessions` for an individual close (rather than a full hub teardown) would hang or misreport the same way.
- **What closes this out:** bind support on `reldex_session_execute` plus an Oracle `ReldexDriverKind` (tracked informally against M2.15; this task did not create a new task row for it, since the owner may want to fold it into an existing one — M2.15 as landed did **not** take either on) for gaps 1–3; a `crates/ffi` fix that retires a session's `hub.sessions` entry on its own `TERMINAL` (not only on hub destroy) for gap 4 — done in M2.15. No shape change expected in `ObjectBrowserModel` itself once either lands.
- **Owner ruling needed:** whether "no freeze with a large schema (measured)" is met-with-limits on the mock-only measurement above, or stays open pending the real-driver run; and whether gap 4 gets its own task/issue against `crates/ffi`.

**M6.2 as implemented, store side only (branch `phase-1/m4-10-m6-2-store`, ADR-0006 amendment).** `crates/workspace` gained `worksheet` (title/text/caret/scroll/tab order, `ON DELETE SET NULL` from its profile), `worksheet_setting` (the real foreign-key target `Scope::Worksheet` settings needed since M2.9, `ON DELETE CASCADE`) and the single-row `layout` (active worksheet/profile, pane sizes, window geometry, both references `ON DELETE SET NULL`) — `save_worksheet`/`load_worksheets`/`delete_worksheet`, `save_layout`/`load_layout`. Non-transactional state only: no session id, no transaction flag, restoring never implies a connection (SPEC §20/§24.16; ADR-0002 D2 — transaction state lives in `DatabaseSession`, not here). Row still `[ ]` todo — what M6.2 needs beyond this:
- [ ] "Store + shell" per the plan row's Outputs column: the shell actually saving/restoring on close/startup (M6.x UI work).
- [ ] M2.11 exposes `save_worksheet`/`load_worksheets`/`delete_worksheet`/`save_layout`/`load_layout` and the `Worksheet`/`WorksheetState`/`Layout`/`PaneSizes`/`WindowGeometry` types over the FFI. (Delivered as `reldex_workspace_save_worksheet`/`reldex_workspace_load_worksheets`/`reldex_workspace_delete_worksheet`/`reldex_workspace_save_layout`/`reldex_workspace_load_layout` and `ReldexWorksheetView`/`ReldexWorksheetList`/`ReldexLayout` — see "M2.11 notes" above; checkbox left for the lead pending review.)
...
10. **Fetch-batch default (M5.6).** — *Recommend* the owner signs off the number the benchmark produces rather than pre-committing one. S14 showed 10,000 rows/batch was ~3.5× *slower* than the best of 100 and 1,000, so intuition is actively wrong here.
    **Status:** Open — not yet decided (deferred to the M5.6 benchmark by design). **Evidence added by S15's fetch-size sweep (2026-09-20, headless + in-app, `phase-1-s15-ffi-spike.md` "Sweeps"):** as information for the owner, not a decision — 1,000 rows/fetch with 2 fetches in flight is the fastest-or-tied-fastest stream, the lowest per-row memory, and the lowest per-batch boundary cost among the sizes that stream well; 100 rows/fetch costs ~20 MB/million rows more; 50,000 rows/fetch costs 30.64 ms of first-row latency; more than 2 fetches in flight buys nothing (n=3 repeats: the four settings' medians span 14 ms against a 16–40 ms per-cell spread). Every number has **zero network latency in it** and must be re-measured on a real network in M5.7, per the report's own recommendation. **Re-asked 2026-09-25 with a corrected diagnosis (ADR-0004 owner-review point (a)):** S15's sweep ran on the mock driver and never exercised the Oracle wire path. On 19c, `oracledb` 26.0.0-beta.3 re-parses a response from byte 0 for every packet, so one fetch costs about the square of its bytes. The same 1,000 rows take 4.4 ms (`s14`), 22 ms (5 texts + 2 dates) or 23 s (4 × `VARCHAR2(4000)`) (`phase-1-m5-1-data/driver-fetch-probe.csv`). The ADR therefore asks for a sign-off on a **bytes-per-round-trip budget** that M5.6 measures, with `results.fetch_rows` kept as an upper bound. M5.2 uses a 256 KiB placeholder until then. **Measured by M5.6 (2026-09-26; `phase-1-fetch-benchmark.md`), for the owner's sign-off:** a budget of **192 KiB per round trip** (`results.round_trip_bytes`) with `results.fetch_rows` = 1,000 as the row bound, on Oracle 19.3 at loopback and 2/10/40 ms injected latency; 23ai, a real WAN and mobile not measured. The wire array stays at 100 rows until the owner rules on ADR-0004 point (g).
