...
| ID | Status | Title | Owner | Inputs | Outputs | Deps | Acceptance | Size |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| M5.1 ★ | `[ ]` todo | **ADR-0004 — Result Store representation, paging and bounded-memory policy** | `opus` | ARCHITECTURE §13 item 6; spike S14 | `docs/decisions/0004-result-store.md` | M1 gate | Decides: retain the fetched prefix in core, fetch-on-demand as the view scrolls, configurable row/byte caps with an explicit "fetched N rows (limit reached)" state; spill/eviction and Arrow explicitly deferred to P3 with the benchmark that would reopen it. **As written (2026-09-25; Proposed, awaiting lead acceptance and owner review):** [ADR-0004](../../decisions/0004-result-store.md) RS1–RS6 — core-owned compacted columnar segments (scaled-`i64` `NUMBER`, exact-size text, LOB ids); the view states demand and the core fetches with one batch of read-ahead; `results.max_rows`/`results.max_bytes` desktop 1,000,000 / 512 MiB, mobile 100,000 / 64 MiB, cursor kept open at the cap; no eviction, spill and Arrow deferred to P3 with reopening benchmarks; C ABI 4 shape for M5.2. Evidence: `phase-1-m5-1-data/` and `crates/db-core/benches/result_store_shapes.rs` | M |
