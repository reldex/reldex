...
| M4.7 | `[~]` in progress — draft PR #51, review pending | DBMS_OUTPUT pane: per-worksheet enable, size, clear, truncation notice | `sonnet` | M2.7 | QML | M2.7 | Off by default (it costs a round trip); truncation is reported, never silent. The pane must also say that the user's own `DBMS_OUTPUT` calls override it: an `ENABLE(2000)` in the user's code overflows at 2,000 bytes even under an "unlimited" pane, and a `DISABLE` in the user's code purges what was buffered. **M2.15 hand-off:** `SERVER_OUTPUT` now arrives ahead of the reply that follows it, with `request == 0`, so route it by session. One statement's output may span several events. Each event owns its `server_output_lines` (release with `reldex_server_output_lines_release`; today's `Bridge` releases them unshown). The pane must show three things: `server_output_dropped` (per-session cap of 256 undrained unsolicited events, also reported on `TERMINAL`), `server_output_invalid_utf8_lines`, and a non-null `error`, which means a read failed and the output is incomplete. **As implemented (2026-09-27, branch `phase-1/m4-7-dbms-output-pane`, draft PR #51; review pending):** three new adapter classes — `SettingsController` (a second `ReldexWorkspace*` handle, generic `resolve`/`setValue`/`clearValue` over any `SettingId`, ADR-0006 P6), `ServerOutputModel` (`QAbstractListModel` over `std::vector<QString>`), `ServerOutputController` (`bridge.serverOutput`, ties both to its own dedicated mock session) — plus additive `SessionController`/`Bridge` wiring for `SERVER_OUTPUT`/`SERVER_OUTPUT_CONFIGURED` and the `reldex::LinesHandle` `handleEvent()` param. No new `SettingId`: `ServerOutputEnabled`/`ServerOutputBuffer` (ABI ids 6/7) already existed and were already pinned; no FFI/header change (`gen-header.sh --check` reports it unchanged). **Bug found and fixed during testing, not by inspection:** `worksheet_setting`'s foreign key on `worksheet` (`crates/workspace/src/store/schema.rs`) means a worksheet-scoped setting *write* silently no-ops for a worksheet id that was only ever generated (`reldex_workspace_new_worksheet_id`), never saved — `SettingsController::ensureWorksheet()` (`reldex_workspace_save_worksheet` with a blank placeholder) closes it, with `ServerOutputController::ready()` now gated on it too. A second, narrower bug in the new truncation test itself (not the product code) was found the same way: flooding statements without draining does not by itself guarantee draining has not already interleaved with production by the time this thread's event loop first turns (no adapter-reachable equivalent of the Rust suite's `reldex_hub_pending_events` barrier exists) — fixed with a fixed `QThread::msleep(500)` between the flood and the first wait, verified reliable over 20 repeated runs after the fix (was flaky before it, in both directions). Tests (`ui/tests/tst_serveroutput.cpp`, `RELDEX_WORKSPACE_IN_MEMORY=1` forced by the suite's own static guard): `enabledAndBufferRoundTripThroughTheSettingsRegistryWithProvenance`, `twoWorksheetsAreIsolatedSettingsAndOutputAlike`, `invalidUtf8LinesAreCountedNotShownAsGarbage`, `outputArrivesBeforeTheReplyThatFollowsIt`, `truncationPastTheMocksCapIsReportedNeverSilent`, `clearEmptiesTheModelAndTheTruncationState`, `oneHundredThousandLinesAppendQuicklyWithoutPerLineObjects`; plus `ui/tests/tst_coreinfo.cpp`'s `dbmsOutputPaneRendersAtCurrentScaleFactor` (1×/2× DPI) and `dbmsOutputPaneVirtualizesLargeOutputWithoutOneItemPerLine`. Measured (RelWithDebInfo, offscreen): appending 100,000 lines to `ServerOutputModel` took 1 ms; a real `ListView` over them instantiated 44 delegate items, never 100,000. Gates: `ui/build.sh --test` and `--release --test` both 100% (14/14); `tools/gates.sh --quick`; `crates/ffi/gen-header.sh --check` (unchanged); `RUSTDOCFLAGS="-D warnings" cargo doc --workspace --no-deps`. Hygiene: the real on-disk store (`%LOCALAPPDATA%\com.reldex.reldex\reldex.sqlite3`) mtime unchanged across every build/test run, confirmed by `ui/build.sh`'s own guard each time. | S |
