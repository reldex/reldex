...
| M5.2 ★ | `[ ]` todo | Result Store implementation in `db-core` + FFI batch lifetime rules | `opus` | M5.1 | `db-core` + `crates/ffi` | M5.1 | Random access O(1) over the fetched prefix; batch release rules tested; no locator ever crosses a thread (K1 test extended). **Follow-up from ADR-0004's review (no new id):** `db-core` releases results after a successful typed `TransactionControl` statement exactly as after the commit/rollback commands (ADR-0002 amendment X1) — lands with M5.2 or M4.3, whichever is first | L |
