...
//! [`reldex_db_driver_api::Secret`], which redacts itself in `Debug`.
//! `tests/no_secrets.rs` proves it on the bytes of a real file.
//!
//! # Threading
//!
...
//! This crate depends on `reldex-db-driver-api` only — not on `db-core`, so
//! `db-core` stays free of SQLite, and not on any driver.
//!
...
pub use profile::{
    Authentication, DatabaseType, Environment, MAX_FIELD_BYTES, MAX_NAME_CHARS, PasswordStorage,
    Profile, ProfileDetails, ProfileEndpoint, ProfileError, ProfileField, ServiceTarget,
    TlsOptions, Transport,
};
pub use settings::{Level, ResolveContext, Resolved};
pub use store::{Loaded, RejectReason, RejectedRow, Scope, Store, StoreError, StoreOptions};
