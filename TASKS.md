...
- [x] M2.7 ★ Server output capability (DBMS_OUTPUT) in contract + driver + core polling when enabled (opus, review mandatory) — done 2026-09-24: server output capability — `set_server_output`/`take_server_output` in the contract, Oracle drain via one anonymous block per round trip (10,000 lines in 5 round trips, 32,767-byte lines byte-exact), core drains after every execute before the reply and makes no driver call while off; independently reviewed (no must-fix; should-fixes landed); ADR-0002 amendment T; follow-ups M2.12/M2.13
...
- [x] M2.9 ★ Settings model: three-level resolution with provenance; profile model; SQLite store (opus, review mandatory) — done 2026-09-25: `crates/workspace` (`reldex-workspace`) — typed settings registry with `worksheet ?? profile ?? application ?? built-in` resolution and provenance (4³-state truth tables); connection profiles keyed by random UUID (= credential key) with no password field, a `treat_as_production` flag, and credential-looking endpoint text refused; one SQLite file (WAL, header identity/version, forward-only migration; foreign, newer and half-written files refused untouched; two open races found by looping and fixed); SID descriptor built by the driver (`sid_endpoint`, live-tested); no secret in the file (byte scan); independently reviewed (accept with follow-ups, all landed); ADR-0006
- [x] M2.10 ★ Credential store: `CredentialStore` trait + Windows Credential Manager implementation (opus, review mandatory) —  — done 2026-09-25: crates/secrets (reldex-secrets) — CredentialStore trait with value-free typed errors; Windows Credential Manager backend (generic credential Reldex/profile/<uuid>, LOCAL_MACHINE, versioned RLDX v1 blob ≤ 2,555 bytes, anything else → Malformed → prompt); NoCredentialStore = prompt each time; resolve_password → PasswordSource (36-row truth table); per-user named lock because Credential Manager loses concurrent writes (0/1,200 lost with the lock); Secret zeroizes; independently reviewed (1 must-fix landed: foreign entries were returned as passwords); real round-trips on Windows CI, 875 tests; ADR-0007 (S5–S7 pending owner confirmation); follow-up M2.14
- [x] M2.11 FFI surface for M2.5–M2.10 + regenerate and verify header (sonnet) —  — done 2026-09-26: ABI 3.0 → 3.1, additive (62 functions added, 0 changed, 0 removed) — session events incl. server output (`invalid_utf8_lines`) and `transaction_possibly_lost`, server-output control, statement splitting, metadata prepare, settings/profiles/store on an FFI-owned service thread (numeric setting ids with provenance; real Oracle `DriverBinding`), credentials (numeric `ReldexCredentialError`/store kind; a password only ever crosses *in*, via `reldex_secret_from_utf8`), history/worksheets/layout; C smoke harness 458 checks on 3 OS, ASan/LSan clean; cbindgen enum-allowlist guard test; independently reviewed twice (2 must-fix: credential enums landed; the pump switch split honestly into M2.15)
...
- [x] M2.15 ★ FFI pump switch to the M2.5 event queue: real `Terminal` on a loss mid-statement, `abandon` relayed through the real `SessionRegistry`, `EXECUTING`/`TRANSACTION_STATE` events, server output before the reply (opus, review mandatory; M2.11 review round 2 must-fix 2; after M2.11) — done 2026-09-27 (PR #48, reviewed twice): the hub owns one M2.5 event queue and the per-session pump thread is gone; real TERMINAL on a loss mid-statement, abandon, EXECUTING and TRANSACTION_STATE events, server output before the reply, hub session count fixed; ABI 3.1→3.2 additive (verified by running the 3.1 smoke harness against the 3.2 library); the boundary's share of a drain rose from ~1% to ~3% while the whole drain stayed within noise (ADR-0003 A37); ReldexProfileError added
...
- [x] M3.2 Connection manager UI: list, create/edit/delete, environment, test-connect (sonnet) — done 2026-09-26 (PR #45, three review rounds): profiles list/create/edit/delete, environment + treat-as-production, role picker, test-connect that resolves the stored password first; reviews found and fixed unguarded tests writing the developer's real store (lazy open + harness guard + build.sh store check), store directory creation on the UI thread (moved into the FFI service thread), a 2× DPI Popup collapse, and a Bridge/ConnectionManager teardown use-after-free (regression tests crash before, pass after)
- [ ] M3.3 ★ Connect flow over the async path, with a bounded timeout and a cancellable "Connecting…" state (opus, review mandatory)
- [x] M3.4 Production indicator: persistent, not colour-only (icon + text + tab badge) (sonnet) — done 2026-09-27 (PR #49, reviewed): icon + text indicator in the status bar, a worksheet-header strip above the editor and a badge on every worksheet tab, driven by one adapter bool (treat_as_production, never the Environment enum) that M3.3 sets on connect; greyscale and 1×/2× DPI tests; re-check placement at M4.1/M4.9 once a real Run affordance exists, and add a tab-badge hit-test/accessibility test before M4.9
...
- [~] M5.2 ★ Result Store implementation in `db-core` + FFI batch lifetime rules (opus, review mandatory) —  — Stage A (core) done 2026-09-26: db-core ResultStore per ADR-0004 — immutable per-batch column segments (scaled-i64 NUMBER when exact, byte-identical Display; bench within 0.7% of Table 1; 0 allocations on the read path over 2M reads), demand-driven fetching one batch ahead, row/byte caps with provenance and mobile defaults, every transaction end → Ended{TransactionEnded} (ADR-0002 X1 implemented, typed COMMIT live-tested), session loss → Failed with LOB cells SessionEnded; independently reviewed (no must-fix; follow-ups landed); live 19c 5/5, 979 tests; Stage B (FFI batch lifetime rules, ABI 4) pending M2.11/M2.15
...
- [x] M6.9 Cold first paint ≈ 800–900 ms (D3D11 device creation ≈ 250 ms + first delegate-instantiation polish ≈ 551 ms) vs `SPEC.md` §19 startup target — investigate fix candidates named in the S15 report (sonnet) —  — done 2026-09-25: investigation complete, no code change adopted — fresh baseline on main is median 576 ms (n=12, CoV 23%) vs S15's 903.55 ms with a byte-identical harness (driver/OS-cache drift, not a fix); `QSG_RENDER_LOOP=basic` gives no material gain; D3D11 sync ≈ 190 ms + first delegate polish ≈ 370 ms at 1,000+ rows reproduce S15's shape; warm path 16 ms unchanged; recommendation for the K2 ruling unchanged (rule on the warm path); measured fix path (pre-warm the result delegate) deferred to M4.x when the real grid ships; raw data under `phase-1-s15-data/m6-9/`
