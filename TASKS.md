...
- [ ] M2.15 ★ FFI pump switch to the M2.5 event queue: real `Terminal` on a loss mid-statement, `abandon` relayed through the real `SessionRegistry`, `EXECUTING`/`TRANSACTION_STATE` events, server output before the reply (opus, review mandatory; M2.11 review round 2 must-fix 2; after M2.11)
